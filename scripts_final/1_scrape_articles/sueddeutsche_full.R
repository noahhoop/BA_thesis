...
get_body <- function(src) {
  src |>
    html_elements("#sz-free-content-id, #metered-content") %>% 
    html_elements("p") %>% 
    html_text2() %>% 
    paste(collapse = " ")
}
...
with_progress({
  ...
  sueddeutsche <- article_urls2  |> future_map(\(.x) scrape_safely(.x, p = p))
})

# sueddeutsche |> 
#   bind_rows() |>
#   filter(body != "") %>% 
#   write.csv("data/data/sz2.csv")

sueddeutsche <- lapply(sueddeutsche, function(df) {
  df %>%
...
write_csv(sz, "data/data/sz2.csv")
